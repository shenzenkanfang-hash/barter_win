...
| K线历史 | KlinePersistence | Redis (List) | 已实现但未集成 |
| 高波动窗口 | 内存 | 内存 | 未标记/未灾备 |
| 订单簿 | OrderBook 内部 | 内存 | 实时更新 |

2.6 KlinePersistence - K线Redis持久化 [待集成]
--------------------------------------------------------------------------------
已定义的模块但未集成到主流程:
...
存储规格:
| 周期 | Redis Key | 最大根数 | 说明 |
|------|-----------|----------|------|
| 1m | kline:1m:{symbol} | 320根 | 约5小时 |
| 15m | kline:15m:{symbol} | 100根 | 约1天 |
| 1d | kline:1d:{symbol} | 100根 | 约100天 |

TODO: 需要集成到 KLineSynthesizer，在K线完成时自动推送

2.7 高波动窗口数据灾备 [待实现]
--------------------------------------------------------------------------------
设计目标: 高波动窗口期间（15min >= 13% 或 1min >= 3%）的数据需要特别标记和存储
...
| 数据 | 存储位置 | 说明 |
|------|----------|------|
| 高波动窗口标记 | Redis Set | 存储高波动窗口的时间范围 |
| 窗口内K线 | Redis List | 独立存储高波动期间的K线 |
| 窗口内指标 | Redis Hash | 高波动期间的关键指标快照 |
| 恢复标记 | Redis String | 断点续传标识 |

Redis Key 设计:
| Key Pattern | 类型 | 说明 |
|------------|------|------|
| high_vol:window:{symbol} | ZSet | 高波动窗口 (score=timestamp) |
| high_vol:kline:{symbol}:{window_id} | List | 窗口内K线 |
| high_vol:indicators:{symbol} | Hash | 窗口内指标快照 |
| recovery:checkpoint:{symbol} | String | 最后checkpoint时间戳 |
...
8.1 为什么保留Redis用于K线存储?
--------------------------------------------------------------------------------
虽然账户/持仓改用内存+SQLite，但Redis仍用于以下场景:

| 用途 | 说明 |
|------|------|
| K线历史 | KlinePersistence 已实现，待集成 |
| 高波动窗口 | 15min高波动期间的K线和指标快照 |
| 灾备恢复 | 断点续传，checkpoint机制 |

Redis Key 用途分类:
| Key 前缀 | 用途 | 数据类型 |
|----------|------|----------|
| kline:{period}: | K线历史 | List |
| high_vol:window: | 高波动窗口标记 | ZSet |
| high_vol:kline: | 窗口内K线 | List |
| high_vol:indicators: | 窗口内指标 | Hash |
| recovery:checkpoint: | 恢复点 | String |
...
9.1 KlinePersistence 集成
--------------------------------------------------------------------------------
状态: 模块已定义，待集成到 KLineSynthesizer

需要完成:
1. 在 KLineSynthesizer 集成 KlinePersistence
2. K线完成时自动推送 Redis
3. 启动时从 Redis 恢复历史K线
...
2. 窗口内K线独立存储
3. 窗口内指标快照
4. 断点续传/checkpoint 机制
5. 重启后恢复高波动状态

9.3 Redis 依赖清理
--------------------------------------------------------------------------------
当前状态:
- KlinePersistence 需要 redis 依赖
- 但未实际使用

决策:
- 保留 Redis 依赖（用于K线存储和灾备）
- SQLite 用于事件记录（不依赖外部服务）
- 账户/持仓用内存 + parking_lot RwLock
